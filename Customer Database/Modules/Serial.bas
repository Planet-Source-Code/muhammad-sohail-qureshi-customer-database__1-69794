...
Public Sub Disk_Serial()
   ...
   For drvNumber = PRIMARY_MASTER To QUARTIARY_SLAVE
      ...
      MDIForm1.List1.AddItem "Drive " & drvNumber
      
      With di
      
         Select Case .bDriveType
            Case 0
               MDIForm1.List1.AddItem vbTab & "[Not present]"
            Case 1
               MDIForm1.List1.AddItem vbTab & "Model:" & vbTab & Trim$(.Model)
               MDIForm1.List1.AddItem vbTab & "Serial No:" & vbTab & Trim$(.SerialNumber)
            MDIForm1.Label17.Caption = Trim$(.SerialNumber)
            Case 2
               MDIForm1.List1.AddItem vbTab & "[ATAPI drive - info not available]"
            Case Else
               MDIForm1.List1.AddItem vbTab & "[drive type not known]"
         End Select
